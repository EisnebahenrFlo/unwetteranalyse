...
import { Activity, AlertOctagon, Zap } from "@/components/icons";
      ...
      <div className="min-h-[260px]">
        ...
        {tab === "nowcast" && (
          <NowcastTab
            ...
            thunderProbMax={nc.thunderProbMax}
            precipMax={nc.precipMaxMmPerH}
            precipSum={nc.precipSumMm}
            hailMax={nc.hailMax}
          />
        )}
...
function Header({
  ...
  const headline =
    worstSevere === "extreme"
      ? "Extremes Unwetter im Fokus"
      : worstSevere === "severe"
        ? "Schweres Unwettersignal heute"
        : worstSevere === "moderate"
          ? "Unwetterpotenzial heute"
          : worstSevere === "minor"
            ? "Markante Wetterlage"
            : "Ruhige Wetterlage";
  return (
    <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3">
      <div
        className={cn(
          ...
          worstSevere === "extreme"
            ? "bg-warn-extreme/15 ring-1 ring-warn-extreme/40"
            : worstSevere === "severe"
              ? "bg-warn-severe/15 ring-1 ring-warn-severe/40"
              : worstSevere === "moderate"
                ? "bg-warn-moderate/15 ring-1 ring-warn-moderate/40"
                : worstSevere === "minor"
                  ? "bg-warn-minor/15 ring-1 ring-warn-minor/40"
      ...
      <div className="flex shrink-0 flex-col items-end gap-1">
        ...
        {nowcastLevel !== "none" && nowcastLevel !== worstSevere && (
          <WarnBadge
            severity={nowcastLevel}
            label={`2h ${labelSev(nowcastLevel)}`}
            className="text-[10px]"
          />
        )}
      </div>
...
function NowTab({
  ...
  hazardSet: ReturnType<typeof buildHazards>;
  alerts: WeatherAlert[];
  bundle: ForecastBundle;
}) {
  const top = hazardSet.hazards.slice(0, 6);
  const officialActive = alerts.filter(
    (a) => a.source !== "dwd" || a.id.startsWith("derived-") === false,
  );
  const firstPoint = bundle.hourly[0];
  const liveScore = firstPoint
    ? severeScore(firstPoint)
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <Stat
          ...
          value={`${officialActive.length}`}
          unit="amtlich"
          tone={officialActive.length ? "moderate" : "none"}
        />
        <Stat
          label="Erkannte Gefahren"
          value={`${hazardSet.hazards.length}`}
          unit="Typen"
          tone={hazardSet.worstSevere !== "none" ? "minor" : "none"}
        />
        <Stat
          label="Peak-Fenster"
          value={
            hazardSet.peakWindow
              ? fmtRange(hazardSet.peakWindow.start, hazardSet.peakWindow.end)
              : "—"
          }
          unit="heute"
          tone={hazardSet.peakWindow ? "minor" : "none"}
        />
      </div>
...
function HazardRow({ h }: { h: Hazard }) {
  return (
    ...
    >
      ...
      <div className="flex shrink-0 flex-col items-end gap-1">
        {h.peakValue && (
          <span className="font-mono text-xs font-semibold text-foreground">{h.peakValue}</span>
        )}
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <Stat
          label="Gewitter Peak"
          value={`${Math.round(thunderProbMax * 100)}`}
          unit="%"
          tone={thunderProbMax >= 0.5 ? "moderate" : thunderProbMax >= 0.3 ? "minor" : "none"}
        />
        <Stat
          label="Regen Spitze"
          value={precipMax.toFixed(1)}
          unit="mm/h"
          tone={precipMax >= 15 ? "moderate" : precipMax >= 5 ? "minor" : "none"}
        />
        <Stat
          label="Regen Σ 2h"
          value={precipSum.toFixed(1)}
          unit="mm"
          tone={precipSum >= 10 ? "moderate" : precipSum >= 3 ? "minor" : "none"}
        />
        <Stat
          label="Böen Peak"
          value={(maxGust * 3.6).toFixed(0)}
          unit="km/h"
          tone={maxGust * 3.6 >= 80 ? "severe" : maxGust * 3.6 >= 50 ? "moderate" : "none"}
        />
      </div>
    ...
    step.level === "extreme"
      ...
      : step.level === "severe"
        ...
        : step.level === "moderate"
          ...
          : step.level === "minor"
            ? "bg-warn-minor"
            : step.precipMmPerH >= 0.5
              ? "bg-primary/60"
              : "bg-muted";
  const rainBg =
    step.precipMmPerH >= 25
      ? "bg-warn-severe"
      : step.precipMmPerH >= 10
        ? "bg-warn-moderate"
        : step.precipMmPerH >= 2.5
          ? "bg-primary"
          : step.precipMmPerH >= 0.2
            ? "bg-primary/50"
            : "bg-muted";
  return (
    <div
    ...
    >
      ...
      <div className="min-w-0">
        <div className="relative h-2.5 w-full overflow-hidden rounded-full bg-muted">
          <div
            className={cn("absolute inset-y-0 left-0 rounded-full", sevBg)}
            style={{ width: `${sevPct}%` }}
          />
        </div>
        <div className="mt-1 flex flex-wrap items-center gap-1 text-[10px]">
          {step.level !== "none" ? (
            <span className="rounded bg-foreground/5 px-1.5 py-0.5 font-semibold uppercase tracking-wide text-foreground">
              {labelSev(step.level)}
            </span>
          ) : null}
          {step.hail !== "none" && (
      ...
      <div className="text-right font-mono text-[11px] tabular-nums">
        ...
        <div className="relative ml-auto h-1 w-12 rounded-full bg-muted">
          <div
            className={cn("absolute inset-y-0 left-0 rounded-full", rainBg)}
...
function TodayTab({
  ...
  const peakStats = {
    ...
    tp: Math.max(
      0,
      ...peakHours.map((p) => (p.weatherCode != null && p.weatherCode >= 95 ? 1 : 0)),
    ),
  };
        ...
        <div className="flex flex-wrap items-baseline gap-2">
          ...
          {peakWin && (
            <span className="font-mono text-sm font-semibold text-foreground">
              {fmtRange(peakWin.start, peakWin.end)}
            </span>
          )}
        </div>
          ...
          {tl.map((h, i) => (
            <div
              ...
              className={cn(
                ...
                h.score.level === "extreme"
                  ? "bg-warn-extreme"
                  : h.score.level === "severe"
                    ? "bg-warn-severe"
                    : h.score.level === "moderate"
                      ? "bg-warn-moderate"
                      : h.score.level === "minor"
                        ? "bg-warn-minor"
                        : "bg-muted",
  ...
  return (
    <div
      className={cn(
        ...
        tone === "extreme"
          ? "border-warn-extreme/50 bg-warn-extreme/10"
          : tone === "severe"
            ? "border-warn-severe/50 bg-warn-severe/10"
            : tone === "moderate"
              ? "border-warn-moderate/50 bg-warn-moderate/10"
              : tone === "minor"
                ? "border-warn-minor/50 bg-warn-minor/10"
    ...
    >
      ...
      <div className="flex items-baseline gap-1">
        <span className="truncate font-mono text-lg font-semibold leading-tight text-foreground">
          {value}
        </span>
