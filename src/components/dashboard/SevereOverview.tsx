...
export function SevereOverview({ bundle }: { bundle: ForecastBundle }) {
  ...
  const fallbackPeak = firstPoint
    ? {
        ...
        score: severeScore(firstPoint),
        thunderProb: 0,
        hail: "none" as const,
        downburst: "none" as const,
      }
    : {
        time: now.toISOString(),
        score: { value: 0, level: "none" as const, reasons: [] },
        thunderProb: 0,
        hail: "none" as const,
        downburst: "none" as const,
      };
  const peak = timeline.reduce(
    (best, item) => (item.score.value > best.score.value ? item : best),
    timeline[0] ?? fallbackPeak,
  );
  const reasons = peak.score.reasons.slice(0, 3);

  return (
    <DataCard
      title="Unwetter-Einschätzung jetzt"
      subtitle="Schnelle Lagebewertung der nächsten 24 Stunden."
      meta={bundle.meta}
    >
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.5fr)]">
        <div className="flex min-w-0 items-center gap-4">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <div
                className="font-mono text-5xl font-semibold leading-none tracking-tight"
            ...
            <div className="mt-1 text-sm font-medium text-foreground">
              {LEVEL_TEXT[summary.level]}
            </div>
            <div className="text-xs text-muted-foreground">
              Peak {formatLiveHour(peak.time, now)}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
          <Metric
            label="Gewitter"
            value={`${Math.round(summary.thunderProbMax * 100)} %`}
            active={summary.thunderProbMax >= 0.25}
          />
          <Metric
            label="CAPE"
            value={summary.capeMax != null ? `${summary.capeMax.toFixed(0)} J/kg` : "—"}
            active={(summary.capeMax ?? 0) >= 500}
          />
          <Metric
            label="Starkregen"
            value={`${summary.precipMaxMm.toFixed(1)} mm/h`}
            active={summary.precipMaxMm >= 15}
          />
          <Metric
            label="Böen"
            value={`${(summary.gustMaxMs * 3.6).toFixed(0)} km/h`}
            active={summary.gustMaxMs >= 14}
          />
        </div>
      </div>
  ...
  return (
    <div
      className={cn(
        ...
        active ? "border-warn-minor/50 bg-warn-minor/10" : "border-border bg-background/60",
      )}
    >
