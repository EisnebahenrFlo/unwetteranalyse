...
export function AlertsSummary({ alerts }: { alerts: WeatherAlert[] }) {
  ...
  return (
    <DataCard
      ...
      action={<Link to="/alerts" className="text-xs text-primary hover:underline">Alle</Link>}
    >
      {sorted.length === 0 ? (
        <EmptyState title="Keine aktiven Warnungen" description="Auch keine eigenen Schwellen ausgelöst." icon={<ShieldCheck className="h-5 w-5" />} />
      ) : (
        <div className="flex flex-col gap-2">
          {sorted.slice(0, 3).map((a) => (
            <div key={a.id} className="grid grid-cols-[auto_minmax(0,1fr)] items-start gap-2 rounded-md border border-border bg-background/50 p-2.5">
              <WarnBadge severity={a.severity} />
              <div className="min-w-0">
                <div className="truncate text-sm font-medium text-foreground">{a.headline}</div>
                {a.description && <div className="line-clamp-2 text-[11px] text-muted-foreground">{a.description}</div>}
