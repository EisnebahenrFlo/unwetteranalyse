    ...
    >
      ...
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-xl border border-border bg-gradient-to-br from-card to-muted/30 px-3 py-2.5">
        ...
        <div className="flex shrink-0 flex-col items-end gap-1">
          ...
          {nc.hailMax !== "none" && <WarnBadge severity={nc.hailMax} label={`Hagel ${labelSeverity(nc.hailMax)}`} className="text-[10px]" />}
        </div>
      </div>

      {/* Zusammenfassung */}
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <Summary label="Gewitter Peak" value={`${Math.round(nc.thunderProbMax * 100)} %`} active={nc.thunderProbMax >= 0.3} />
        <Summary label="Regen Spitze" value={`${nc.precipMaxMmPerH.toFixed(1)} mm/h`} active={nc.precipMaxMmPerH >= 5} />
        <Summary label="Regen Σ 2 h" value={`${nc.precipSumMm.toFixed(1)} mm`} active={nc.precipSumMm >= 3} />
        <Summary label="Böen Spitze" value={`${windKmh(maxGust(nc.steps))} km/h`} active={maxGust(nc.steps) * 3.6 >= 50} />
      </div>
    ...
    <div className={cn(
      ...
      <div className="min-w-0">
        <div className="relative h-2.5 w-full overflow-hidden rounded-full bg-muted">
          <div className={cn("absolute inset-y-0 left-0 rounded-full", LEVEL_BG[step.level])} style={{ width: `${sevPct}%` }} />
        </div>
        <div className="mt-1 flex items-center gap-1.5 text-[10px]">
          {step.level !== "none" && (
            <span className={cn("rounded px-1.5 py-0.5 font-semibold uppercase tracking-wide", LEVEL_PILL[step.level])}>
          ...
          {step.thunderProb >= 0.3 && (
            <span className="rounded bg-warn-moderate/15 px-1.5 py-0.5 font-semibold uppercase tracking-wide text-warn-moderate">
              ⚡ {Math.round(step.thunderProb * 100)} %
            </span>
          )}
      ...
      <div className="text-right font-mono text-[11px] tabular-nums">
        ...
        <div className="relative ml-auto h-1 w-12 rounded-full bg-muted">
          <div className={cn("absolute inset-y-0 left-0 rounded-full", rainColor(step.precipMmPerH))} style={{ width: `${precipPct}%` }} />
        </div>
      </div>
      <div className="text-right font-mono text-[11px] tabular-nums">
...
function Summary({ label, value, active }: { label: string; value: string; active: boolean }) {
  return (
    <div className={cn("min-w-0 rounded-lg border px-2.5 py-1.5", active ? "border-warn-minor/60 bg-warn-minor/10" : "border-border bg-background/60")}>
      <div className="truncate text-[10px] uppercase tracking-wide text-muted-foreground">{label}</div>
      <div className="truncate font-mono text-base font-semibold tabular-nums" style={{ fontFamily: "var(--font-mono)" }}>{value}</div>
    </div>
  );
}
