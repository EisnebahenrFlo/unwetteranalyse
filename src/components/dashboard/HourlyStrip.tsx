...
import { formatLiveHour, formatTemp, formatPrecip } from "@/lib/weather/format";
          ...
          {hours.map((h) => {
            ...
            return (
              <div key={h.time} className={cn("flex w-16 shrink-0 flex-col items-center gap-1 rounded-md px-1 py-2 text-center hover:bg-accent/50", isCurrentHour(h.time, now) && "bg-accent")}> 
                <div className="h-7 text-[10px] leading-tight text-muted-foreground">{formatLiveHour(h.time, now)}</div>
