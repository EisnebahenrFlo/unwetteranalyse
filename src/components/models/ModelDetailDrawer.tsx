...
function bewertung(r: ModelRankRow): string {
  if (r.worstScore >= 70) return "Modell zeigt klare Unwetter-Signatur: hohe CAPE und/oder schwere Böen.";
  if (r.worstScore >= 45) return "Markantes konvektives Signal — Gewitter mit Begleiterscheinungen möglich.";
    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="max-h-[88vh]">
        ...
        <div className="grid grid-cols-2 gap-2 px-4 sm:grid-cols-4">
          ...
          <KV label="CAPE max" value={row.capeMax != null ? `${row.capeMax.toFixed(0)} J/kg` : "—"} />
        ...
        <div className="px-4 pb-6 pt-4">
          <div className="mb-2 text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Score-Verlauf (24 h)</div>
          <div className="h-40 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                ...
                />
                <YAxis tick={{ fontSize: 10 }} width={28} stroke="currentColor" className="text-muted-foreground" domain={[0, 100]} />
                <Tooltip
                  labelFormatter={(v) => new Date(v as string).toLocaleString("de-DE", { hour: "2-digit", minute: "2-digit" })}
                  contentStyle={{ fontSize: 11, borderRadius: 8, border: "1px solid var(--border)", background: "var(--popover)" }}
                />
                <Area type="monotone" dataKey="score" stroke="currentColor" fill="url(#scoreGrad)" strokeWidth={1.5} className="text-foreground" isAnimationActive={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
...
function KV({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-border/60 bg-muted/30 px-3 py-2">
      ...
      <div className="mt-0.5 truncate text-sm font-semibold tabular-nums" style={{ fontFamily: "var(--font-mono)" }}>{value}</div>
    </div>
  );
}
