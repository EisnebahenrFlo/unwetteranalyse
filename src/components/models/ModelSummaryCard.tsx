...
export function ModelSummaryCard({
  ...
  const uncLabel =
    summary.uncertainty === "low" ? "gering" : summary.uncertainty === "mid" ? "mittel" : "erhöht";
  return (
    <Card className="flex flex-col gap-4 p-5">
      <div className="flex items-center justify-between gap-3">
        <div className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
          Modelllage auf einen Blick
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 border-t border-border/60 pt-4">
        ...
        <KPI
          label="Modell-Konsens"
          value={`${summary.signalingCount}/${summary.modelCount}`}
          sub={`${consensusPct} %`}
        />
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 text-[11px] text-muted-foreground">
        ...
        {updatedAt && (
          <span>
            Aktualisiert {formatRelative(new Date(updatedAt) > now ? now.toISOString() : updatedAt)}
          </span>
        )}
      </div>
    </Card>
