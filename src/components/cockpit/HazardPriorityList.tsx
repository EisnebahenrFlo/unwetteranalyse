 ...
 */
export function HazardPriorityList({ bundle, officialAlerts }: { bundle: ForecastBundle; officialAlerts: WeatherAlert[] }) {
  ...
  if (items.length === 0 && officialAlerts.length === 0) {
    return (
      <Card className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-3 p-4 md:p-5">
        <div className="grid h-10 w-10 place-items-center rounded-md bg-muted text-base">✓</div>
        <div className="min-w-0">
          <div className="text-sm font-semibold text-foreground">Keine markanten Gefahren in den nächsten 24 Stunden.</div>
          <div className="text-[11px] text-muted-foreground">Weder eigene Schwellen noch amtliche Warnungen aktiv.</div>
        </div>
      </Card>
    );
  }
    ...
    <Card className="flex flex-col divide-y divide-border/60 p-0">
      {officialAlerts.length > 0 && (
        <div className="flex items-center gap-2 bg-warn-moderate/5 px-4 py-2 text-[12px] text-warn-moderate">
          <span className="rounded bg-warn-moderate/15 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider">Amtlich</span>
          <span className="truncate">
            {officialAlerts.length} aktive Warnung{officialAlerts.length === 1 ? "" : "en"} vom DWD im Gebiet.
          </span>
        </div>
      )}
      {items.map((h) => <HazardRow key={h.id} h={h} />)}
    </Card>
  );
}
    ...
    <div className="grid grid-cols-[6px_auto_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/40">
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          ...
          {h.kind === "heat" && <span className="rounded bg-heat/15 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-heat">Hitze</span>}
          {h.kind === "cold" && <span className="rounded bg-cold/15 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-cold">Kälte</span>}
          {h.kind === "info" && <span className="rounded bg-info/15 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-info">Hinweis</span>}
        </div>
      ...
      <div className="flex shrink-0 items-center gap-2">
        <div className="hidden flex-col items-end gap-0.5 sm:flex">
          {h.peakValue && <span className="font-mono text-[12px] font-semibold tabular-nums text-foreground">{h.peakValue}</span>}
          <span className="text-[10px] text-muted-foreground">Konf. {h.confidence}/5</span>
        </div>
        {h.kind !== "heat" && h.kind !== "cold" && h.kind !== "info" && <WarnBadge severity={h.severity} className="hidden md:inline-flex" />}
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
