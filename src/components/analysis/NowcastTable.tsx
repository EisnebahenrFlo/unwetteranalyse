  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border">
      ...
      <div className="divide-y divide-border/60">
        {steps.map((s) => <Row key={s.time} step={s} maxRain={maxRain} daily={daily} />)}
      </div>
    </div>
  );
}

function Row({ step, maxRain, daily }: { step: NowcastStep; maxRain: number; daily?: DailyPoint[] }) {
  ...
  const conf = Math.round((step.rain.confidence + step.wind.confidence + step.thunder.confidence + step.convection.confidence) / 4);
  const confDot = conf >= 75 ? "bg-emerald-500" : conf >= 45 ? "bg-amber-500" : "bg-rose-500";
  return (
    <div className={cn("grid grid-cols-[58px_36px_44px_minmax(0,1fr)_64px_44px_30px] items-center gap-2 px-2 py-1.5", step.minutesFromNow === 0 && "bg-accent/40")}>
      <div className="font-mono text-[11px] leading-tight tabular-nums" style={{ fontFamily: "var(--font-mono)" }}>
      ...
      <div className="flex items-center gap-1 text-[10.5px]">
        {lightning > 0 ? (
          <span className="rounded bg-warn-severe/20 px-1.5 py-0.5 font-semibold text-warn-severe">⚡ {lightning}</span>
        ) : thunderPct >= 30 ? (
          <span className="rounded bg-warn-moderate/15 px-1.5 py-0.5 font-semibold text-warn-moderate">⚡ {thunderPct}</span>
        ) : (
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2 text-[11px]">
          <div className="relative h-1.5 flex-1 overflow-hidden rounded-full bg-muted">
            <div className="absolute inset-y-0 left-0 rounded-full bg-primary" style={{ width: `${rainPct}%` }} />
          </div>
          <span className="font-mono text-[10.5px] tabular-nums text-foreground" style={{ fontFamily: "var(--font-mono)" }}>
            {mm >= 0.05 ? `${mm.toFixed(1)} mm/h` : "—"}
          </span>
        </div>
      </div>
      <div className="text-right font-mono text-[11px] tabular-nums text-foreground" style={{ fontFamily: "var(--font-mono)" }}>
        {gKmh != null ? gKmh.toFixed(0) : "—"}
      </div>
      <div className="text-right">
        <span className={cn("inline-flex items-center justify-center rounded-md px-1.5 py-0.5 font-mono text-[11px] font-semibold tabular-nums", c.bg, c.text)} style={{ fontFamily: "var(--font-mono)" }}>
          {step.total}
        </span>
      </div>
      <div className="flex items-center justify-end">
        <span className={cn("inline-block h-2 w-2 rounded-full", confDot)} title={`Vertrauen ${conf}/100`} />
      </div>
    </div>
  );
}