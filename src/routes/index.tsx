  ...
  head: () => ({
    meta: [
      { title: "Cockpit — ForecastHub" },
      {
        name: "description",
        content:
          "Internes Analyse-Cockpit: Lage, Gefahren, Kurzfrist, Live-Signale, Trend und Systemstatus.",
      },
    ],
  }),
  component: Dashboard,
...
function Dashboard() {
  ...
  if (forecast.error || !forecast.data) {
    return (
      <ErrorState
        message={forecast.error?.message ?? "Forecast nicht verfügbar."}
  ...
  const bsMeta = bsCurrent.data
    ? {
        source: "bright-sky" as const,
        updatedAt: bsCurrent.data.observedAt,
        resolutionKm: 1,
        uncertainty: "Punktwert der nächsten DWD-Station.",
      }
    : bundle.meta;
  ...
  const sources: SourceEntry[] = [
    ...
    },
    {
      ...
      meta: bsAlerts.data
        ? { source: "bright-sky", updatedAt: new Date().toISOString() }
        : undefined,
      ok: bsAlerts.data != null,
      note:
        officialAlerts.length === 0 ? "Keine aktiven Warnungen" : `${officialAlerts.length} aktiv`,
    },
    {
      id: "dwd-radar",
      label: "DWD · Radarkomposit",
      description: "Animiertes Niederschlagsechobild über Deutschland.",
      meta: latestRadarFrame
        ? { source: "dwd", updatedAt: latestRadarFrame, resolutionKm: 1 }
        : undefined,
    ...
    <div className="flex flex-col gap-6 md:gap-10">
      <DataFreshnessStrip
        entries={[
          ...
          {
            label: "Radar",
            updatedAt: latestRadarFrame,
            warnAfterMin: 20,
            ok: !radarFrames.isError,
          },
          {
            label: "Warnungen",
            updatedAt: bsAlerts.data ? new Date().toISOString() : undefined,
            warnAfterMin: 30,
            ok: bsAlerts.data != null,
          },
        ]}
