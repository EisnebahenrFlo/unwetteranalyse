    ...
    <div className="flex flex-col gap-3">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Offizielle DWD-Warnungen via Bright Sky (DACH) plus eigene Schwellen aus der Forecast-Analyse.
        </p>
      </div>
      ...
      <DataCard title="Offizielle Warnungen (DWD / Bright Sky)">
        ...
        {!official.isLoading && (official.data?.length ?? 0) === 0 && (
          <EmptyState title="Keine aktiven offiziellen Warnungen" icon={<ShieldCheck className="h-5 w-5" />} />
        )}
          ...
          {[...(official.data ?? [])]
            ...
            .map((a) => (
              <article key={a.id} className="rounded-md border border-border bg-background/50 p-3">
                ...
                {a.description && <p className="mt-2 whitespace-pre-line text-xs text-foreground/90">{a.description}</p>}
                {a.instruction && <p className="mt-2 rounded bg-muted px-2 py-1.5 text-[11px] text-muted-foreground">{a.instruction}</p>}
              </article>
            ))}
        </div>
      </DataCard>

      <DataCard title="Eigene Schwellen-Analyse (DWD-orientiert)" subtitle="Aus dem Forecast der nächsten 48 h abgeleitet">
        {forecast.isLoading && <Skeleton className="h-20 w-full" />}
        {derived.length === 0 && !forecast.isLoading && (
          <EmptyState title="Keine Schwelle ausgelöst" description="Forecast bleibt innerhalb der DWD-orientierten Werte." icon={<ShieldAlert className="h-5 w-5" />} />
        )}
        <div className="flex flex-col gap-2">
          {derived.map((d) => (
